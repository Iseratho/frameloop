...
labels_abstracts_df = pd.DataFrame(fl(news_df["abstract"].tolist()), index=news_df.index)
...
dimensions_abstracts_df = pd.DataFrame(fd(news_df["abstract"].tolist()), index=news_df.index)
...
amr_graphs_abstracts = fs(news_df["abstract"].tolist(), error_type=penman.decode("(amr-error)"))
structure_abstracts_df = pd.DataFrame(list(map(penman.encode, amr_graphs_abstracts)), index=news_df.index)
